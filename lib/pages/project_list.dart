...
class ProjectListPage extends ConsumerWidget {
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              TextField(controller: projectNameController, decoration: const InputDecoration(labelText: 'Project Name')),
              TextField(controller: projectInfoController, decoration: const InputDecoration(labelText: 'Project Info')),
            ],
          ),
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                await _updateProject(context, ref, project['id'], projectNameController.text.trim(), projectInfoController.text.trim());
  ...
  Future<void> _deleteProject(BuildContext context, WidgetRef ref, String projectId) async {
    try {
      ...
      if (response == null) {
        ...
        if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Project deleted successfully')));

        return;
      }

      // Handle unexpected non-null responses
      if (response is Map && response.containsKey('error')) throw Exception(response['error']['message']);

      // Safely show success for non-error responses
      if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Project deleted successfully')));
    } catch (e) {
      if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error deleting project: $e')));
    }
  }
  ...
  Future<void> _updateProject(BuildContext context, WidgetRef ref, String projectId, String name, String info) async {
    try {
      final response = await Supabase.instance.client.from('project_list').update({'project_name': name, 'project_info': info, 'updated_by': Supabase.instance.client.auth.currentUser?.id, 'updated_at': DateTime.now().toIso8601String()}).eq('id', projectId);
  ...
  Future<void> _createProject(String name, String info) async {
    await supabase.from('project_list').insert({'project_name': name, 'project_info': info, 'created_by': supabase.auth.currentUser?.id});
  }
}
